...
# Funkcia zisti ci item patri k hladanej entite
...
def write_type_of_ent(file_, array) -> None:
    if "organizace" in file_.name:
        file_.write("organization\t")
...
# Najprv skusi ceske potom angilcke... Po neuspechu vypise TAB
...
def write_start_of_ent(file_, array, entity_d) -> None:
    if array == sub_of_organisation:
        try:
            file_.write(entity_d["claims"]["P571"][0]["mainsnak"]["datavalue"]["value"]["time"] + "\t")
        except KeyError:
            file_.write("\t")
    elif array == sub_of_occurence:
        try:
            file_.write(entity_d["claims"]["P580"][0]["mainsnak"]["datavalue"]["value"]["time"] + "\t")
        except KeyError:
            file_.write("\t")
    elif array == sub_of_artist:
        try:
            file_.write(entity_d["claims"]["P569"][0]["mainsnak"]["datavalue"]["value"]["time"] + "\t")
        except KeyError:
            file_.write("\t")


# Funkcia vypise do suboru koniec/zrusenie/... entity
def write_end_of_ent(file_, array, entity_d) -> None:
    if array == sub_of_organisation:
        try:
        ...
        try:
            file_.write(entity_d["claims"]["P570"][0]["mainsnak"]["datavalue"]["value"]["time"] + "\t")
        except KeyError:
            file_.write("\t")
...
def write_location_of_ent(file_, entity_d, array) -> None:
    if array.__eq__(sub_of_artist):
        return
    try:
...
def write_dbpedia_url_of_ent(file_, entity_d) -> None:
    try:
        ...
        file_.write("http://dbpedia.org/page/" + en_name + "\t")
    except Exception as e:
        print(e)
        file_.write("\t")
...
def write_influenced_by(file_, entity_d) -> None:
    try:
        file_.write(str(entity_d["claims"]["P737"][0]["mainsnak"]["datavalue"]["value"]) + "\t")
    except KeyError:
        file_.write("\t")
...
def write_genres(file_, entity_d) -> None:
    try:
        file_.write(str(entity_d["claims"]["P136"][0]["mainsnak"]["datavalue"]["value"]) + "\t")
    except KeyError:
        file_.write("\t")
...
def generic(file, ent, array, entity) -> None:
    ...
    write_type_of_ent(file, array)  # type
...
def others(file, ent, array) -> None:
    if array[0] in sub_of_organisation.values() or array[0] in sub_of_occurence.values():
        write_start_of_ent(file, array, ent)  # inception
        write_end_of_ent(file, array, ent)  # cancelled
        write_location_of_ent(file, ent, array)  # location
        write_type_of_type(file, array[0])  # org_type

    elif array[0] in sub_of_artist.values():
...
def get_atr(file, ent, array, help_array) -> int:
    try:
        p = ent["labels"]["cs"]["value"]    # If entity does not have czech label
    except KeyError:
        raise KeyError

    try:
        l = ent["claims"]["P279"]       # If entity is also subclass
        return 0
    except KeyError:
        l = 0

    try:
        entity = WikidataItem(ent)      # If valid item
    except ValueError:
        return 0
...
def write_log_file(org, occ, art, i, ncl, d_time, f_time, log_file):
    ...
    ff.write("{}s: Found {} occurrences among {} entities [entities/s: {:.2f}]\n".format(
